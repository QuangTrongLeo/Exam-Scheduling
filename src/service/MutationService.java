...
public class MutationService {
	...
	private void resolveRoomConflicts(Individual individual) {
		// Dùng ConflictKey thay vì String
		Map<ConflictKey, List<ClassSession>> conflictMap = new HashMap<>();
		...
		for (Gene gene : individual.getGenes()) {
			for (ClassSession session : gene.getClassSessions()) {
				// Tạo Key object từ model.ConflictKey
				ConflictKey key = new ConflictKey(session.getTimeSlot().getDay(), session.getTimeSlot().getPeriod(),
		...
		// Xử lý các nhóm có xung đột (Size > 1)
	...
	private void moveSessionToEmptyRoom(ClassSession session, Individual individual) {
		...
		// Check an toàn: Nếu không có phòng nào phù hợp loại này thì bỏ qua
		if (targetRooms.isEmpty())
			return;

		// Tìm danh sách ID các phòng ĐANG BẬN tại khung giờ này trong toàn bộ cá thể
		Set<Integer> occupiedRoomIds = getOccupiedRoomIdsAtTime(individual, currentSlot);

		// Tìm danh sách phòng TRỐNG (Candidate rooms)
		...
		if (!availableRooms.isEmpty()) {
			...
			// Đường cùng (Full phòng): Chọn đại một phòng khác phòng hiện tại
			Room currentRoom = session.getRoom();
			Room randomRoom = currentRoom;
			int attempts = 0;

			// Thử 10 lần tìm phòng khác ID phòng hiện tại
	...
	// Lấy ID các phòng đang bận tại một khung giờ cụ thể
